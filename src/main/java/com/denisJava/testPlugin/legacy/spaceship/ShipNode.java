...
public abstract class ShipNode {
    ...
    public ShipNodeStatus fullUpdate(ShipStatus sh, int id, ShipNodeStatus _status) {
        ShipNodeStatus status = _status;

        boolean breakCheck = true;
        if (this instanceof ShipNodeBreakable nodeBreakable) {
            if (nodeBreakable.durability < 0 && _status != ShipNodeStatus.REPAIRING) {
                status = ShipNodeStatus.BROKEN;
                breakCheck = false;
            } else if (nodeBreakable.durability >= 0) {
                status = ShipNodeStatus.OK;
            } else {
                breakCheck = false;
            }
        }
        ...
        if (shouldStop) status = ShipNodeStatus.DISABLED;
            ...
            if (_status.shouldUpdate && canWork() && breakCheck)
                status = update(sh, _status);
        }
